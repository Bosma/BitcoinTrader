  ...
  auto open_callback = [&]() {
    ...
    for (auto &m : mktdata) {
      auto OHLC_is_fetched = [&]() {
        exchange_log->output("BACKFILLING OKCOIN FUTS " + to_string(m.first.count()) + "m BARS");
        bool success = okcoin_futs->backfill_OHLC(m.second.period, m.second.bars.capacity());
        if (success)
          exchange_log->output("FINISHED BACKFILLING OKCOIN FUTS " + to_string(m.first.count()) + "m BARS");
        else
